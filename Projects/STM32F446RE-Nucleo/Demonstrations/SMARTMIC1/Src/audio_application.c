...
void AudioProcess(void)
{
  ...
  if (!Internals.LOCK)
  {
    CCA02M2_AUDIO_IN_PDMToPCM(CCA02M2_AUDIO_IN_INSTANCE, (uint16_t *)aPDMBuffer, aPCMBufferIN);  /* PDM to PCM*/
...
/**
  ...
  * @param  None
  * @retval AUDIO_OK if no problem occurred, AUDIO_ERROR otherwise
  */
uint8_t AudioDevices_Init(void)
{
  ...
  if (CCA02M2_AUDIO_IN_Init(CCA02M2_AUDIO_IN_INSTANCE, &MicParams) != AUDIO_OK)
  {
    return AUDIO_ERROR;
  }

  /* Initialize DBNoise peripheral */
  if (BSP_AUDIO_IN_PDMToPCM_DB_Noise_Init(AUDIO_IN_SAMPLING_FREQUENCY, 4, 1) != AUDIO_OK)
  {
    return AUDIO_ERROR;
  }
  uint16_t volume_temp = aVolumeLin[UserAudioStatus.OutputStatus.Volume];
  ...
  if (CCA01M1_AUDIO_OUT_Init(CCA01M1_AUDIO_OUT_INSTANCE, &OutParams) != AUDIO_OK)
  {
    return AUDIO_ERROR;
  }
...
uint8_t StartMicAcquisition(void)
{
  ...
  return CCA02M2_AUDIO_IN_Record(CCA02M2_AUDIO_IN_INSTANCE, (uint8_t *)aPDMBuffer, AUDIO_IN_BUFFER_SIZE);
...
void AudioStatus_Init(void)
{
  ...
  UserAudioStatus.GeneralStatus.SamplingFreq = AUDIO_IN_SAMPLING_FREQUENCY;
  UserAudioStatus.BeamStatus.Direction = 1;
  ...
  UserAudioStatus.OutputStatus.Volume = AUDIO_VOLUME_OUT;
...
uint32_t InitializeApplication(void)
{
  ...
  CCA02M2_AUDIO_IN_SetVolume(CCA02M2_AUDIO_IN_INSTANCE, (uint32_t)UserAudioStatus.GeneralStatus.Volume);
  ...
  if (Internals.AudioOutStatus != (AUDIOOUT_STATUS_STOP || AUDIOOUT_STATUS_PAUSE))
  {
    ...
    HAL_NVIC_DisableIRQ(AUDIO_OUT_INTERRUPT);
    Internals.AudioOutRdPtr = 0;
    Internals.AudioOutWrPtr = (AUDIO_OUT_BUFFER_SIZE / 4);
      ...
      || UserAudioStatus.OutputStatus.Status == AUDIOOUT_STATUS_MICS)
  {
    ...
    {
      CCA01M1_AUDIO_OUT_Play(CCA01M1_AUDIO_OUT_INSTANCE, (uint8_t *)aAudioOutBuffer, AUDIO_OUT_BUFFER_SIZE);
    }
...
void PeriodicAudioStatusManager(void)
{
  ...
  if (AudioStatusLastChanged & DOMAIN_GENERAL)
  {
    ...
    if (UserAudioStatus.GeneralStatus.Volume != InternalStatus->GeneralStatus.Volume)
    {
      ...
      CCA02M2_AUDIO_IN_SetVolume(CCA02M2_AUDIO_IN_INSTANCE, InternalStatus->GeneralStatus.Volume);
    }
  ...
  {
    CCA01M1_AUDIO_OUT_SetVolume(CCA01M1_AUDIO_OUT_INSTANCE, aVolumeLin[InternalStatus->OutputStatus.Volume]);
    /*copy*/
