...
{
  HAL_IncTick();
  if (ASR_led_timeout > 0)
  {
    ASR_led_timeout--;
  }
  else
  {
    if (toggle_led_timeout > 0)
    {
      static short time = 25;
      toggle_led_timeout --;
      ...
      {
        LedsToggle(toggle_led_mask);
        if (toggle_led_mask != fixed_led_mask)
        {
          LedsOff(0xFF - toggle_led_mask - fixed_led_mask);
        }
        else
        {
          LedsOff(0xFF - toggle_led_mask);
        }
